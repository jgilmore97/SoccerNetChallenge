...
class SoccerNetClips(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split="train", 
                 ...
                 chunks_per_epoch=6000, subset=None):
        self.path = path 
        ...
        # If subset is provided, take a slice of listGames
        if subset is not None:
            self.listGames = self.listGames[:subset]
            logging.info(f"Using a subset of {len(self.listGames)} games for the dataset.")
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            if len(self.game_anchors[class_selection]) > 0:
                break
